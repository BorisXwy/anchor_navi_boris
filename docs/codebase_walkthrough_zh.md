...
**2026-09-12 补记：走路层加了两条到达/停滞后处理规则（M4，对应 `failure_analysis_95_plain_zh.md` 病根 F；只在 `_execute_rgb_only` 里，旧几何路径不动）。** (a) **门类形式到达后滑行**：`PointNavigationRequest` 新增 `instruction_form`（RGB-only 策略层传 `stage["form"]`，这是指令形式不是几何，不在违禁字段里）；当 form ∈ `ARRIVAL_COAST_FORMS = {EXIT_REGION, ENTER_REGION, TRAVERSE_PORTAL_REGION, SELECT_PORTAL}` 时，`ordinary_arrival`/`terminal_consensus` 判到达之后再发最多 `arrival_coast_forward_steps`（profile 里 3）个 `move_forward`——因为底边停簇离开画面时相机还离所选点约 0.9 m，门类句子会停在门槛里侧；每一步算 `_rgb_motion_score`，一旦低于 `stall_motion_threshold` 就停止滑行并把那条记录标 `reversal_skip="arrival_coast_stall"`。滑行记录只进 `action_history`（带 `phase: "arrival_coast"`），**不进 `record["steps"]`**（那里每条都要有 9 个 track，`validate_single_point_run.py` 会查）；汇总在 `record["arrival_coast"]`。(b) **前进停滞后原地探测**：命中 `stall_forward_frames` 不再立刻结束，而是先把这段没动的前进标 `reversal_skip="forward_stall"`（`mark_stalled_forwards`，向后走到第一条真动了的前进或上一次探测为止），然后原地转 `stall_recovery_turn_deg`（30° = 2 个 turn 命令，偶数次探测向左、奇数次向右，镜像旧路径的交替逻辑），每帧都 `tracker.step`（不变式：**每渲染一帧恰好一次 tracker.step，不跳不重**——正常循环在 break 时会跳过末尾那次，探测路径要自己补上停滞帧），重置 streak 后 `continue` 让控制律（0.8 像素角 + 0.2 policy）自己转回目标；最多 `stall_recovery_max_probes`（2）次，之后仍停滞才以 `rgb_forward_stall` 结束。探测转向带 `phase: "stall_recovery_probe"`，判定 prompt 的 `action_summary` 把它们单列为 `stall_recovery_turn_command_count`、不计入左右转计数（净角≈0，不能冒充 TURN_* 的转弯证据）。探测和滑行都不消耗 `max_steps` 循环预算，但额外动作有硬上限（4 + 3，记在 `record["extra_physical_actions"]`）；关键帧仍按 `linspace` 均匀抽，帧数变多会稀释真实运动帧。(c) **倒放跳过没动的前进**：`plan_action_reversal` 遇到带 `reversal_skip` 的记录直接跳过，回溯 attempt 记 `skipped_no_motion_forward_count`——之前停滞触发的回溯把撞墙时那几步「前进」当真走了，回程多退 0.6–1 m。开关：`--arrival-coast-steps` / `--stall-recovery-probes`（默认 None = profile 值；0 = 旧行为；三个评测入口都转发），旧路径的 legacy profile 也用 `stall_recovery_max_probes` 这个键名，所以 CLI 默认必须是 None 而不是 2，否则会把 v16 的 8 盖掉。2 条单 EP 冒烟（40、739）里三条规则都触发了：门类句到达离所选点的距离 0.83→0.17、0.98→0.32、1.06→0.69 m；一次「停滞→左探→停滞→右探→停滞」序列后倒放跳过 9 条前进，落点误差 0.86 m——剩余误差来自贴墙蹭行的前进（RGB 变化 22 但位移只有 5 cm，停滞规则抓不到，见 `docs/forward_stall_collision_handling_zh.md`「抓不到什么」）。单元测试 `tests/test_rgb_only_executor_recovery.py`。
...
**2026-09-12 补记二：v3 家族（M5，病根 D/E；holdout 未通过预注册门槛，默认仍是 v2）。** `v3_sector_relation_evidence` / `v3b_relation_rules_relaxed` = v2 全部内容 + 追加在同一个 `form_rules` 槽位（五个 JSON 块之后，解析器不受影响）的四段：EVIDENCE PROTOCOL（schema 多四个必填字段 `landmark_visible_current` / `landmark_sector_current`（front|left|right|rear|not_visible|not_applicable）/ `relation_satisfied` / `transition_observed`，要求先填再定 status；`completed` 但 `relation_satisfied=false` 由 `normalize_rgb_only_completion_result` 确定性降为 unknown、confidence ≤ 0.49，与"空边不能 completed"同型，override 记在结果的 `validation_overrides`，四个字段记在 `relation_evidence`）、REASONS THAT NEVER JUSTIFY UNKNOWN（以前进结尾/未静止/无法测距/下一句未开始/步数少）、按 `form + secondary_forms` 只渲染命中的 RELATION RULE（`RGB_ONLY_COMPLETION_RELATION_RULES`，无命中用 GENERIC；v3b 用 `..._V3B` 表放宽 PASS_LANDMARK 延展地标、TURN_* 不要求走廊居中、STOP_WAIT 任意扇区近距/地面地标/出门即停、EXIT_REGION 隔门看见源房间不算未出）、FALSE-POSITIVE GUARD + 置信度量表。schema 与归一化抽成 classmethod `rgb_only_completion_schema(version)` / `normalize_rgb_only_completion_result(version, result, sanitized_actions)`，在线判定与 `replay_rgb_only_judge_round.py` 共用；`HeuristicBackend` 按 schema 补齐新字段。评分口径、冻结标签、门槛与结果见 `docs/judge_calibration_m5_protocol_zh.md`：holdout 上 v3b F1 0.715 / FP 12 / 末句 NEG FP 7 对 v2 的 0.700 / 11 / 4，三条门槛都不满足，剩余误差是地标实例身份（M6）。
...
2. 按 `reversed(H)` 逐条回放，`turn_left`↔`turn_right` 互换，`move_forward` 不变；
   （2026-09-12 起：执行器标了 `reversal_skip` 的记录——停滞段里没产生 RGB 变化的 `move_forward`、滑行时撞停的那一步——在回放时跳过，attempt 记 `skipped_no_motion_forward_count`；细节见模块 4 的 2026-09-12 补记 (c)。）
